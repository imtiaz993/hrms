...
export function AdjustEntryPopup({ onClose, employees }: AdjustEntryPopupProps) {
    ...
    const fetchEntry = async () => {
        ...
        try {
            ...
            if (data) {
                setEntry(data);
                setClockIn(data.clock_in ? format(parseISO(data.clock_in), "HH:mm") : "");
                setClockOut(data.clock_out ? format(parseISO(data.clock_out), "HH:mm") : "");
            }
        } catch (err) {
    ...
    const calculateFlags = (ci: string, co: string) => {
        if (!selectedEmployee) return { late: false, early: false };
        ...
        if (ci) {
            ...
            if (ci > standardStart.substring(0, 5)) {
                is_late = true;
            }
        }
        ...
        if (co) {
            ...
            if (co < standardEnd.substring(0, 5)) {
                is_early_leave = true;
            }
        }
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            if (fullClockIn && fullClockOut) {
                const start = new Date(fullClockIn);
                const end = new Date(fullClockOut);
