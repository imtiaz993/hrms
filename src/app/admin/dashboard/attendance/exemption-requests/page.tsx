...
import { CheckCircle2, XCircle, Clock, User, Calendar, FileText, Loader2, Search, ChevronLeft, ChevronRight } from "lucide-react";
import { formatTime, getCurrentTime, toPKTISO, parsePKT } from "@/lib/time-utils";
        ...
        try {
            ...
            if (action === "approved") {
                ...
                if (existingEntry) {
                    ...
                    await supabase.from("time_entries").insert([{ ...entryPayload, created_at: toPKTISO(getCurrentTime()) }]);
                }
            }
                ...
                <div className="space-y-4">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-[11px] text-slate-500 uppercase tracking-wider">{req.employees?.department}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex flex-col">
                                                    <span className="font-medium text-slate-900">{format(parsePKT(req.date), "MMM dd, yyyy")}</span>
                                                    <span className="text-[11px] text-slate-500">{format(parsePKT(req.date), "EEEE")}</span>
                                                </div>
                                            </td>
                                                ...
                                                <div className="flex flex-col gap-1">
                                                    <div className="flex items-center gap-2 text-[11px]">
                                                        ...
                                                        <span className="font-bold text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded">
                                                            {req.new_clock_in ? format(parsePKT(req.new_clock_in), "HH:mm") : "--"} - {req.new_clock_out ? format(parsePKT(req.new_clock_out), "HH:mm") : "--"}
                                                        </span>
                                                    </div>
                                                    <div className="flex items-center gap-2 text-[10px] opacity-60 group-hover:opacity-100 transition-opacity">
                                                        ...
                                                        <span className="text-slate-500 line-through">
                                                            {req.old_clock_in ? format(parsePKT(req.old_clock_in), "HH:mm") : "N/A"} - {req.old_clock_out ? format(parsePKT(req.old_clock_out), "HH:mm") : "N/A"}
                                                        </span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="max-w-[200px] truncate text-slate-600 text-xs italic" title={req.reason}>
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap text-right">
                                                {req.status === "pending" ? (
                                                    <div className="flex items-center justify-end gap-2">
                                                        ...
                                                        >
                                                            {processingId === req.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <CheckCircle2 className="h-3.5 w-3.5 mr-1.5" />}
                                                            Approve
                                                        </Button>
                                                        <Button
                                                            size="sm"
                                                    ...
                                                    <div className="text-[11px] text-slate-400 italic">
                                                        Processed on {format(parsePKT(req.updated_at || req.created_at), "MMM dd")}
                                                    </div>
                                                )}
                                            </td>
                                        </tr>
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between px-2">
                        <div className="text-xs text-slate-500">
                            Showing <span className="font-semibold text-slate-900">{showingFrom}</span> to <span className="font-semibold text-slate-900">{showingTo}</span> of <span className="font-semibold text-slate-900">{total}</span> total entries
                        </div>
                        <div className="flex items-center gap-3">
