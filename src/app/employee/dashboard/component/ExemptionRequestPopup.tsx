...
export function ExemptionRequestPopup({ currentUser, onClose, onSuccess, existingRequests }: ExemptionRequestPopupProps) {
    ...
    useEffect(() => {
        if (selectedDate) {
            ...
            const hasPending = existingRequests.some(r => r.date === selectedDate && r.status === "pending");
            if (hasPending) {
    ...
    const fetchEntry = async () => {
        ...
        try {
            ...
            if (data) {
                setEntry(data);
                setNewClockIn(data.clock_in ? format(parseISO(data.clock_in), "HH:mm") : "");
                setNewClockOut(data.clock_out ? format(parseISO(data.clock_out), "HH:mm") : "");
            } else {
                setNewClockIn("");
                setNewClockOut("");
            }
    ...
    const calculateFlags = (ci: string, co: string) => {
        ...
        if (ci && currentUser.standard_shift_start) {
            if (ci > currentUser.standard_shift_start.substring(0, 5)) {
                is_late = true;
            }
        }

        if (co && currentUser.standard_shift_end) {
            if (co < currentUser.standard_shift_end.substring(0, 5)) {
                is_early_leave = true;
            }
        }
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        try {
            const fullNewIn = newClockIn ? `${selectedDate} ${newClockIn}:00` : null;
            const fullNewOut = newClockOut ? `${selectedDate} ${newClockOut}:00` : null;
                            ...
                            <>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="grid gap-2">
                                        ...
                                        />
                                        <div className="text-[10px] text-slate-400">Current: {entry?.clock_in ? format(parseISO(entry.clock_in), "HH:mm") : "N/A"}</div>
                                    </div>
                                    <div className="grid gap-2">
                                        ...
                                        />
                                        <div className="text-[10px] text-slate-400">Current: {entry?.clock_out ? format(parseISO(entry.clock_out), "HH:mm") : "N/A"}</div>
                                    </div>
                                </div>
