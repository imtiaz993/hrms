...
import { Plus, History, XCircle, Clock } from "lucide-react";
                            ...
                            {requests.map((req) => (
                                <div key={req.id} className="p-3 border rounded-xl bg-white shadow-sm space-y-2">
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <div className="text-sm font-medium">{format(parseISO(req.date), "MMM dd, yyyy")}</div>
                                    ...
                                    <div className="grid grid-cols-2 gap-2 text-xs">
                                        <div className="p-2 bg-slate-50 rounded-lg">
                                            <div className="text-slate-400 mb-1 font-medium uppercase tracking-tight" style={{ fontSize: '10px' }}>Old Times</div>
                                            <div className="flex flex-col">
                                                <span>In: {req.old_clock_in ? format(parseISO(req.old_clock_in), "HH:mm") : "--:--"}</span>
                                                <span>Out: {req.old_clock_out ? format(parseISO(req.old_clock_out), "HH:mm") : "--:--"}</span>
                                            </div>
                                        </div>
                                        <div className="p-2 bg-blue-50 rounded-lg">
                                            <div className="text-blue-400 mb-1 font-medium uppercase tracking-tight" style={{ fontSize: '10px' }}>New Times</div>
                                            <div className="flex flex-col text-blue-700">
                                                <span>In: {req.new_clock_in ? format(parseISO(req.new_clock_in), "HH:mm") : "--:--"}</span>
                                                <span>Out: {req.new_clock_out ? format(parseISO(req.new_clock_out), "HH:mm") : "--:--"}</span>
                                            </div>
                                        </div>
                                    </div>
